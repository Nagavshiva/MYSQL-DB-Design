CREATE TABLE `Guvi`(
    `Sign-up` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    `Login` INT NOT NULL
);
...
ALTER TABLE
    `Guvi` ADD CONSTRAINT `guvi_login_foreign` FOREIGN KEY(`Login`) REFERENCES `GUVI`(`Zen Class`);
ALTER TABLE
...
ALTER TABLE
    `GUVI` ADD CONSTRAINT `guvi_rewards_foreign` FOREIGN KEY(`Rewards`) REFERENCES `Zen Class`(`Students`);
ALTER TABLE
